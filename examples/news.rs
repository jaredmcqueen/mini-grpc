...
use news::NewsRequest;
use sherpa::news_streamer_client::NewsStreamerClient;
use tonic::Request;
...
async fn connect_and_stream() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    let mut client = NewsStreamerClient::connect("http://[::1]:10000").await?;
    let mut stream = client
        .get_news(Request::new(NewsRequest { news: true }))
        .await?
        .into_inner();
